...
func TestClaudeCodeValidator_SecurityMonitorWithoutBillingBlock(t *testing.T) {
	...
	// 本用例验证严格 deep-validation 下对安全监视器分类器请求的识别/拒绝逻辑；
	// fork 在 init() 中把 policy 换成 relaxed（仅 UA 判定），这里临时切回严格策略，
	// 结束后恢复原 policy，保持上游断言原样有效。
	prevPolicy := claudeCodeDeepPolicy
	SetClaudeCodeDeepValidationPolicy(strictClaudeCodeDeepValidation)
	t.Cleanup(func() { SetClaudeCodeDeepValidationPolicy(prevPolicy) })
	// </fork>
	monitorPrompt, err := os.ReadFile("testdata/security_monitor_system_prompt.txt")
	require.NoError(t, err)
