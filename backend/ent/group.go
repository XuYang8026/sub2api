...
type Group struct {
	...
	// OpenAI OAuth 请求缺 instructions 时是否跳过 Codex CLI base prompt 注入（发空串）
	SkipCodexDefaultInstructions bool `json:"skip_codex_default_instructions,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldSkipCodexDefaultInstructions:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field skip_codex_default_instructions", values[i])
			} else if value.Valid {
				_m.SkipCodexDefaultInstructions = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("skip_codex_default_instructions=")
	builder.WriteString(fmt.Sprintf("%v", _m.SkipCodexDefaultInstructions))
	builder.WriteString(", ")
