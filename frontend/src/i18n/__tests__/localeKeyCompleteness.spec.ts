...
// <fork:i18n-merge>
// Fork-only locale extensions live in *-fork-ext.ts and are deep-merged into
// the upstream locale at runtime (see ../index.ts). Mirror that here so keys
// referenced by fork sidecar components are not reported as missing.
import enForkExt from '../locales/en-fork-ext'
import zhForkExt from '../locales/zh-fork-ext'
// </fork>

type LocaleValue = Record<string, unknown>

// <fork:i18n-merge>
function deepMergeLocale(base: LocaleValue, ext: LocaleValue): LocaleValue {
  for (const [k, v] of Object.entries(ext)) {
    const b = base[k]
    if (v && typeof v === 'object' && !Array.isArray(v) && b && typeof b === 'object' && !Array.isArray(b)) {
      deepMergeLocale(b as LocaleValue, v as LocaleValue)
    } else {
      base[k] = v
    }
  }
...
const en = deepMergeLocale(structuredClone(enBase) as LocaleValue, enForkExt as LocaleValue)
const zh = deepMergeLocale(structuredClone(zhBase) as LocaleValue, zhForkExt as LocaleValue)
// </fork>
