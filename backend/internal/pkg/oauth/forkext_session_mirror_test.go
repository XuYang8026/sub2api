...
func TestForkSessionMirror_SessionSetOnOneInstanceIsVisibleOnAnother(t *testing.T) {
	...
	podA.Set("sid-1", &OAuthSession{State: "state-1", CodeVerifier: "verifier-1", Scope: "org:create_api_key", CreatedAt: time.Now()})
	...
	require.Equal(t, "org:create_api_key", got.Scope)
}
